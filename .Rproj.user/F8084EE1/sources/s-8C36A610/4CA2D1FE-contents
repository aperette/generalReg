...
conf_interval = function(x, c, level=0.95){
  par=sapply(c,list)
  ...
  m=sqrt(Matrix::diag(solve(x$fisher)))
  p=(1+level)/2
  z=qnorm(p)
  int=matrix(c(c-m*z,c+m*z),nrow=length(c))
  rownames(int)=names(x$theta)
  colnames(int)=c(paste0(100*(1-p)," %"),paste0(100*(p)," %"))
  return(int)
}

fisher_inf = function(x, c){
  par=sapply(c,list)
  mF=x$functions$function_F(par,x$data)
  H=x$functions$function_H(x$functions$function_sigma(par,x$data))
  m=Matrix::t(mF)%*%H%*%mF
...
estruturar = function(char,lista,asArray=F){
  ...
  k_max=max(length(c1$ori),length(c2$ori))

  for(k in 1:length(c0$ori))
    char=stringr::str_replace_all(char,c0$ori[k],c0$int[k])
  for(k in 1:length(c1$ori))
    char=stringr::str_replace_all(char,c1$ori[k],c1$int[k])
  for(k in 1:length(c2$ori))
